...
from .basicaertools_gpu import *

logger = logging.getLogger(__name__)


class DmSimulatorGPUPy_Base(BackendV1):
        ...
        if self._initial_densitymatrix is None:
            if self._custom_densitymatrix is None:
                ...
                for i in range(self._number_of_qubits - 1):
                    self._densitymatrix = np.kron(np.array([1, 0, 0, 1], dtype=self._precision), self._densitymatrix)
