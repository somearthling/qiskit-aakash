...
from .dm_simulator_gpu_base import DmSimulatorGPUPy_Base as DmSimulatorPy_Base

logger = logging.getLogger(__name__)

class DmSimulatorGPUPy(DmSimulatorPy_Base):
    MAX_QUBITS_MEMORY = int(log2(local_hardware_info()["memory"] * (1024**3) / 16))

    DEFAULT_CONFIGURATION = {
        "backend_name": "dm_simulator_gpu",
    ...
    def __init__(self, configuration=None, provider=None):
        ...
        print("Default backend: {}".format(jax.default_backend()))
        print("Available backends: {}".format(jax.devices()))
